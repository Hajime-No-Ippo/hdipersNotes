...
---
title: CS144 Lab 9: Turing Machines & Automata
...
1.  **保存文件**：
    * 将上面的代码保存为 `posts/lab9.md`。

2.  **图片处理 (Image Handling)**：
    * 我在 Markdown 里写了类似 `*(注：请在 JSFLAP 中绘制以上逻辑...)*` 的提示。
    * 如果你有真实的截图，请把文件名为 `Q1.2_TM_aaa.png` 等的图片放入仓库的 `images/` 文件夹（如果还没建这个文件夹，建一个）。
    * 然后在 Markdown 里用 `![TM](images/Q1.2_TM_aaa.png)` 替换掉 Mermaid 图表，或者两者并存。
    * *如果没有截图，现有的 Mermaid 图表已经足够清晰，可以直接发布！*

3.  **Git Push**：
    * `git add .`
    * `git commit -m "Add Lab 9 notes"`
    * `git push`

4.  **Cloudflare Pages**：
    * 它会自动检测到 Push，自动部署。
    * 等待 30 秒，刷新你的 `hdipers.22abad.com`。
