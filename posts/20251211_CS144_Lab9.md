...
> **Notation Key:**
>
> - `0/0,R`: Read `0`, Write `0`, Move **Right**.
> - `0/0,S`: Read `0`, Write `0`, **Stay** (No move).
...
- **Q4.1 Binary to Decimal**:
  `1011_2` → `1 * 2^3 + 0 * 2^2 + 1 * 2^1 + 1 * 2^0` = `8 + 0 + 2 + 1` = **11_10**
- **Q4.2 Hex to Decimal**:
  `1A3_16` → `1 * 16^2 + 10 * 16^1 + 3 * 16^0` = `256 + 160 + 3` = **419_10**
- **Q4.3 Decimal to Binary**:
  `45_10` → `32 + 8 + 4 + 1` → `1 * 2^5 + 0 * 2^4 + 1 * 2^3 + 1 * 2^2 + 0 * 2^1 + 1 * 2^0` → **101101_2**
  [END]
...
> **符号说明：**
>
> - `0/0,R`: 读 `0`, 写 `0`, 向 **右 (Right)** 移。
> - `0/0,S`: 读 `0`, 写 `0`, **停 (Stay)** 在原地。
> - `B`: **空白符 (Blank)** (纸带上的空位)。
